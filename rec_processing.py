...
class QuizzrProcessorHead:
    ...
    def __init__(self, qtpm: QuizzrTPM, directory: str, config: dict, dict_name: str,
        ...
        self.logger = logger or logging.getLogger(__name__)
        self.qp = QuizzrProcessor(qtpm.database, self.rec_dir, config, submission_file_types, self.logger)
...
def start_watcher(db_name, tpm_config, firebase_app_specifier, api, rec_dir, queue_dir, error_dir, proc_config, queue,
                  dict_name, submission_file_types=None, logger=None):
    ...
    qph = QuizzrProcessorHead(qtpm, rec_dir, proc_config, dict_name, submission_file_types, logger.getChild("qp"))
